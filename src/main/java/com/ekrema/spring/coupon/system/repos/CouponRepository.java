...
public interface CouponRepository extends JpaRepository<Coupon, Integer> {
    ...
    @Query(value = "SELECT coupons.id,coupons.company_id,coupons.category,coupons.title,coupons.description,coupons.start_date,coupons.end_date,coupons.amount,coupons.price,coupons.image\n" +
            " FROM coupons\n" +
            "INNER JOIN customers_coupons ON coupons.id=customers_coupons.coupons_id WHERE customer_id=?", nativeQuery = true)
    List<Coupon> findByCustomer(int customerId);

    @Query(value = "SELECT coupons.id,coupons.company_id,coupons.category,coupons.title,coupons.description,coupons.start_date,coupons.end_date,coupons.amount,coupons.price,coupons.image\n" +
            " FROM coupons\n" +
            "INNER JOIN customers_coupons ON coupons.id=customers_coupons.coupons_id WHERE customer_id=? AND coupons.category=?", nativeQuery = true)
    List<Coupon> findByCustomerAndCategory(int customerId, String category);

    @Query(value = "SELECT coupons.id,coupons.company_id,coupons.category,coupons.title,coupons.description,coupons.start_date,coupons.end_date,coupons.amount,coupons.price,coupons.image\n" +
            " FROM coupons\n" +
            "INNER JOIN customers_coupons ON coupons.id=customers_coupons.coupons_id WHERE customer_id=? AND price<=?", nativeQuery = true)
